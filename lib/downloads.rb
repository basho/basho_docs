...
class Downloads
  ...
  def self.pull_data(project, version)
    # Don't pull older versions of Riak since they have a weird format. None of
    # the other projects have this problem.
    return if project == 'riak' && version.sub(/\.\w+$/, '').to_f <= 1.1
