...
# We're going to wrap the DOM manipulation inside an AJAX callback that's inside
# an onReady() callback, so hopefully we won't prevent users from interacting
# with the load DOM while the JSON is fetched and the manipulation is performed.
generateVersionLists = () ->
