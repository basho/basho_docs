...
This command controls Riak CS's [garbage collection](/riak/cs/2.0.1/cookbooks/garbage-collection) system.
...
begin listening on the host and port specified in your [configuration files](/riak/cs/2.0.1/cookbooks/configuration/reference).
...
When a member's weight is updated, that weight is stored in the [master member](/riak/cs/2.0.1/cookbooks/supercluster/#The-Master-Member) and cached in Riak CS. Riak CS fetches weights from the master member only periodically. The
