...
aliases:
  ...
  - /riak/cs/latest/cookbooks/command-line-tools/
---
...
This command controls Riak CS's [garbage collection]({{<baseurl>}}riak/cs/3.0.0/cookbooks/garbage-collection) system.
...
begin listening on the host and port specified in your [configuration files]({{<baseurl>}}riak/cs/3.0.0/cookbooks/configuration/reference).
...
More information can be found in [Riak CS Supercluster Support]({{<baseurl>}}riak/cs/3.0.0/cookbooks/supercluster).
...
When a member's weight is updated, that weight is stored in the [master member]({{<baseurl>}}riak/cs/3.0.0/cookbooks/supercluster/#the-master-member) and cached in Riak CS. Riak CS fetches weights from the master member only periodically. The
