...
toc: true
aliases:
    - /riakts/1.6.0/using/querying/basic-querying
    - /riakts/1.6.0/using/querying/guidelines
...
[SELECT]: /riak/ts/1.6.0/using/querying/SELECT#iso_8601
...
It is possible to use ISO 8601-compliant date/time strings rather than integer timestamps in SELECT statements. Please see [SELECT](/riak/ts/1.6.0/using/querying/select/#iso-8601) for an example or [Time Representations](/riak/ts/1.6.0/using/timerepresentations/) for more information.
{{% /note %}}
...
Blob data should be queried using integers in base 16 (hex) notation, preceded by `0x` using riak shell or by providing any block of data (e.g. binary, text or JSON) through a Riak client library. Blob data may only use strict equality (=, !=).

We do not recommend using blob columns in primary keys, due to limitations in the `list_keys` API.
