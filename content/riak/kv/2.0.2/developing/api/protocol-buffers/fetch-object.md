...
[`RpbPutResp`](/riak/kv/2.0.2/developing/api/protocol-buffers/store-object), respectively):
...
* `usermeta` --- This field stores user-specified key/value metadata
  ...
  send metadata of this sort are structured like this:
    ...
    `RpbPair` messages are also used to attach [secondary indexes](/riak/kv/2.0.2/developing/usage/secondary-indexes) to objects (in the optional
    `indexes` field).
