...
The bucket's name (`bucket`) must be specified. The [bucket type]({{<baseurl>}}riak/kv/2.0.2/using/cluster-operations/bucket-types) parameter (`type`) is optional. If it is not specified,
...
[configuration file]({{<baseurl>}}riak/kv/2.0.2/configuring/reference/#default-bucket-properties) documentation.
