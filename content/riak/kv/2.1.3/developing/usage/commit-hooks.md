...
Pre- and post-commit hooks are applied at the [bucket](/riak/kv/2.1.3/learn/concepts/buckets) level,
...
documentation on [object deletion](/riak/kv/2.1.3/using/cluster-operations/object-deletion)) to determine whether a delete is
occurring.
...
- `fail` --- The atom `fail` will cause Riak to fail the write and send a 403 Forbidden error response (in the [HTTP API](/riak/kv/2.1.3/developing/api/http)) along with a generic error message about why the write was blocked.
