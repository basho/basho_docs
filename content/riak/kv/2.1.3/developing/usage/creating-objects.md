...
which bears the `animals` [bucket type]({{<baseurl>}}riak/kv/2.1.3/developing/usage/bucket-types).
...
Now, run the same read operation in [Reading Objects]({{<baseurl>}}riak/kv/2.1.3/developing/usage/reading-objects). If the write operation was successful, you should be able to successfully read the object. Your Riak cluster is no
longer empty!
...
[`default`]({{<baseurl>}}riak/kv/2.1.3/developing/usage/bucket-types) will be applied.
