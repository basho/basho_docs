...
As an example, let's store an object containing information about a dog named Rufus. We'll store that object in the key `rufus` in the bucket `dogs`, which bears the `animals` [bucket type](/riak/kv/2.0.8/using/cluster-operations/bucket-types).
...
[`default`](/riak/kv/2.0.8/developing/usage/bucket-types) will be applied.
