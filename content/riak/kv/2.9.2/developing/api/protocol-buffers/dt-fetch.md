...
`type` | The [Using Bucket Types]({{<baseurl>}}riak/kv/2.9.2/using/cluster-operations/bucket-types) of the bucket in which the Data Type is stored, _not_ the type of Data Type (i.e. counter, set, or map)
...
The response to a fetch request ([`DtFetchReq`]({{<baseurl>}}riak/kv/2.9.2/developing/api/protocol-buffers/dt-fetch)) is a `DtFetchResp` message.
...
would send a [vclock]({{<baseurl>}}riak/kv/2.9.2/learn/glossary/#vector-clock) for standard KV updates.
