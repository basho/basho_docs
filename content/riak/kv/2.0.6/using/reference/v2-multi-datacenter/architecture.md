...
2. The site node in the secondary cluster initiates fullsync replication
   ...
   the primary cluster
3. The site and listener nodes iterate through each [vnode](/riak/kv/2.0.6/learn/glossary/#Vnode) in their respective clusters and compute a hash for
...
same [ring size](/riak/kv/2.0.6/learn/concepts/clusters/#The-Ring); if you are using fullsync
replication, every bucket's [`n_val`](/riak/kv/2.0.6/developing/app-guide/replication-properties#N-Value-and-Replication) must be the same in both the
