...
toc: true
aliases:
  - /riak/3.0.8/dev/references/http/store-search-index
  - /riak/kv/3.0.8/dev/references/http/store-search-index
---

Creates a new Riak Search [index]({{<baseurl>}}riak/kv/3.0.8/developing/usage/search/#simple-setup).
...
will create a new Search index that uses the [default Search schema]({{<baseurl>}}riak/kv/3.0.8/developing/usage/search-schemas/#the-default-schema), i.e. `_yz_default`.
...
the schema to use. If you've [stored a schema]({{<baseurl>}}riak/kv/3.0.8/developing/usage/search-schemas/#custom-schemas) called `my_custom_schema`, the following `PUT`
...
More information can be found in [Using Search]({{<baseurl>}}riak/kv/3.0.8/developing/usage/search).
