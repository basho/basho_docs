...
A:
  ...
  You can also set auto-expire using the [Memory](/riak/kv/2.0.0/setup/planning/backend/memory) storage backend, but it will be limited by RAM.
...
A:
  ...
  You can find more details in the [Configuration Files](/riak/kv/2.0.0/configuring/reference) document.
...
A:
  ...
  For further information see [System Planning](/riak/kv/2.0.0/setup/planning/start/#network-configuration-load-balancing).
...
A:
  ...
  See [vector clocks](/riak/kv/2.0.0/learn/concepts/causal-context#vector-clocks) for more information.
...
A:
  Riak does not implement any form of schema validation. A pre-commit hook can be used in this scenario but would need to be written by your development team. You can read more about [commit hooks](/riak/kv/2.0.0/developing/usage/commit-hooks) in the docs. This document provides two pre-commit hook examples, one in Erlang that restricts objects that are too large and one in Javascript that restricts non-JSON content.
...
A:
  The [Erlang Riak Client](/riak/kv/2.0.0/developing/client-libraries) gives you several options for how to manage connections. You can set these when starting a `riakc_pb_socket` process or by using the `set_options` function.
...
A:
  ...
  More on [Bucket Properties](/riak/kv/2.0.0/developing/usage).
...
A:
  ...
  You can read more on `app.config` in [Configuration Files](/riak/kv/2.0.0/configuring/reference).
...
A:
  You'll need to use a [MapReduce](/riak/kv/2.0.0/developing/usage/mapreduce/) job for this.
...
A:
  Aside from the performance considerations of doing a full-bucket [MapReduce](/riak/kv/2.0.0/developing/usage/mapreduce/), searching is a form of filtering, which should be done in the map phase.
