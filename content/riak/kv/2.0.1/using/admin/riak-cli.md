...
aliases:
  ...
  - /riak/kv/2.0.1/ops/running/tools/riak
---
...
Checks whether the [configuration file](/riak/kv/2.0.1/configuring/reference/) is valid. If so, `config is OK` will be included in the output.
...
More detailed information can be found in the [`riak-admin`](/riak/kv/2.0.1/using/admin/riak-admin/#top) documentation.

## config

Provides information about the current [configuration](/riak/kv/2.0.1/configuring/reference/) of a Riak node, i.e. the parameters and values in the node's `riak.conf` or `app.config` (depending on which configuration system is being used).
