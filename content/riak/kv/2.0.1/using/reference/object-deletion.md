...
is a trivial process. In an [eventually consistent](/riak/kv/2.0.1/learn/concepts/eventual-consistency), [clustered][concept clusters] system like Riak, however,
object deletion is far less trivial because objects live on multiple
[nodes](/riak/kv/2.0.1/learn/glossary/#nodes), which means that a deletion process must be chosen to determine when an object can be removed from the storage backend.
...
* A client attempts to read the object, Riak senses that there are
  divergent replicas and initiates a repair process (either [read repair](/riak/kv/2.0.1/learn/concepts/active-anti-entropy/#read-repair) or [active anti-entropy](/riak/kv/2.0.1/learn/concepts/active-anti-entropy/),
...
1. A tombstone object (`<<>>`) is written to N [vnodes][glossary vnode], with N
   defined by [`n_val`](/riak/kv/2.0.1/developing/app-guide/replication-properties#n-value-and-replication)
