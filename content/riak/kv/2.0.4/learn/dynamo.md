...
>Like Dynamo, Riak KV employs consistent hashing to partition and replicate data around the ring. For the consistent hashing that takes place in riak_core, Basho chose the SHA1 hash. See [Consistent Hashing](http://docs.basho.com/riak/kv/2.0.4/learn/glossary/#consistent-hashing) in our docs.
>
>Riak KV uses vector clocks for object versioning. Scroll down to section 4.4 to read up on this in depth.
>
>Riak KV makes use of gossiping in the same way that Dynamo does: to communicate ring state and node membership. See [Gossip Protocol](http://docs.basho.com/riak/kv/2.0.4/learn/glossary/#gossiping) in our docs.
>
...
>
>Riak KV has two APIs: an [HTTP API](/riak/kv/2.0.4/developing/api/http/) and a [Protocol Buffers API](/riak/kv/2.0.4/developing/api/protocol-buffers/)
...
>[Further Reading on Partitioning in Riak KV](http://docs.basho.com/riak/kv/2.0.4/learn/concepts/clusters/) and [All about the Riak KV Ring](http://docs.basho.com/riak/kv/2.0.4/learn/concepts/clusters/#the-ring)
...
>
>[Vector Clock on Riak KV Glossary](/riak/kv/2.0.4/learn/glossary/#vector-clock)
>
...
>
>[REST API](http://docs.basho.com/riak/kv/2.0.4/developing/api/http/)
>
>[Writing Data](/riak/kv/2.0.4/developing/usage/creating-objects/)
>
>[Reading Data](/riak/kv/2.0.4/developing/usage/reading-objects/)
...
>Same for Riak KV. Reconciling divergent versions in Riak KV is called [Read Repair](/riak/kv/2.0.4/learn/concepts/replication/#read-repair).
...
>[Hinted handoff](/riak/kv/2.0.4/learn/glossary/#hinted-handoff) is built into Riak KV's core.
>
>You can glimpse at Riak KV's preference list (or *preflist*) calculation in the [Replication](/riak/kv/2.0.4/developing/usage/replication/) walkthrough.
...
>[Multi Datacenter Replication](/riak/kv/2.0.4/using/reference/v3-multi-datacenter/architecture/) is implemented in the commercial extension to Riak KV, called [Riak KV Enterprise Edition](http://basho.com/products/riak-kv/).
...
>This section is well expressed in [Adding and Removing Nodes](/riak/kv/2.0.4/using/cluster-operations/adding-removing-nodes/) and [Failure Scenarios](/riak/kv/2.0.4/learn/concepts/eventual-consistency/).

#### 4.8.1 Ring Membership

>Riak KV operators can trigger node management via the [riak-admin command-line tool](/riak/kv/2.0.4/using/admin/riak-admin/).
...
>Riak KV's ring state holds membership information, and is propgated via [gossiping](/riak/kv/2.0.4/learn/glossary/#gossiping), including random reconciliation, defaulting to once a minute.
...
>
> See [*The Node Join Process*](/riak/kv/2.0.4/using/cluster-operations/adding-removing-nodes/#joining-nodes-to-form-a-cluster) for more.
...
>Riak KV ships with various [backend options](/riak/kv/2.0.4/setup/planning/backend/). [Bitcask](/riak/kv/2.0.4/setup/planning/backend/bitcask/) is the default, but [LevelDB](/riak/kv/2.0.4/setup/planning/backend/leveldb/)and Main [Memory](/riak/kv/2.0.4/setup/planning/backend/memory/) are also used heavily in production (in that order). You can also use more than one backend in production via the [[Multi]] backend configuration.
> 
>Bitcask is a fast and reliable choice, but does have some limitations at very large scales. For larger clusters, you may want to choose LevelDB (which also supports [secondary indexes](/riak/kv/2.0.4/developing/usage/secondary-indexes/)). The Memory backend is an excellent choice when speed is important and durability is not. It also has TTL support.
...
>Riak KV implements [Read Repair|](/riak/kv/2.0.4/learn/concepts/replication/#read-repair).
...
>Much of this section relates to benchmarks run against Dynamo. You can run [Basho Bench](/riak/kv/2.0.4/using/performance/benchmarking/) against your own Riak cluster to discover your own optimal values.
...
>Riak follows a SHA1 based consistent hashing for [partitioning](/riak/kv/2.0.4/learn/concepts/replication/#understanding-replication-by-example).
...
> 
> See [The Node Join Process](/riak/kv/2.0.4/using/cluster-operations/adding-removing-nodes/#joining-nodes-to-form-a-cluster) and [Replacing a Node](/riak/kv/2.0.4/using/cluster-operations/replacing-node/)
...
>
> See [Load Balancing](/riak/kv/2.0.4/configuring/load-balancing-proxy/) for more information.
...
>Many [client libraries](/riak/kv/2.0.4/developing/client-libraries/) provide built-in node request coordination.
>
...
>This is equally true for Riak. As mentioned above, consider running [Basho Bench](/riak/kv/2.0.4/using/performance/benchmarking/) to help discover your optimal setup. Nothing will give you better numbers than real experimentation.
