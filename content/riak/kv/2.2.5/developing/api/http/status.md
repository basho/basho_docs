...
toc: true
aliases:
  - /riak-docs/riak/2.2.5/dev/references/http/status
  - /riak-docs/riak/kv/2.2.5/dev/references/http/status
---
...
The output of `/stats` contains the output of `riak-admin status` detailed in the [Inspecting a Node]({{<baseurl>}}riak/kv/2.2.5/using/cluster-operations/inspecting-node) doc, plus the below stats generated by the Riak Core application.
