...
toc: true
aliases:
  - /riak-docs/riak/2.2.5/ops/running/logging
  - /riak-docs/riak/kv/2.2.5/ops/running/logging
---
...
As of Riak KV 2.2.5 a few of the log messages may contain newline
...
* `file` --- Console logs will be emitted to a file. This is Riak's
    ...
    `./log/console.log` on an installation from [source]({{<baseurl>}}riak/kv/2.2.5/setup/installing/source), but will differ on platform-specific installation,
...
* `console` --- Console logs will be emitted to standard output, which
    can be viewed by running the [`riak attach-direct`]({{<baseurl>}}riak/kv/2.2.5/using/admin/riak-cli/#attach-direct) command
