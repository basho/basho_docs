...
status of a node. This data is available by running the [`riak-admin status`](/riak/kv/2.2.2/using/admin/riak-admin/#riak-admin-status) command. That data now
includes statistics specific to strongly consistent operations.

A full listing of these stats is available in [Inspecting a Node](/riak/kv/2.2.2/using/cluster-operations/inspecting-node).
