...
aliases:
  ...
  - /riak/kv/3.0.4/dev/taste-of-riak/object-modeling-erlang
---
...
We'll be using the bucket `Users` to store our data. We won't be [using bucket types]({{<baseurl>}}riak/kv/3.0.4/developing/usage/bucket-types) here, so we don't need to specify one.
