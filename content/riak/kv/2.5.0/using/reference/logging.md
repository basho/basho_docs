...
toc: true
aliases:
  - /riak/2.5.0/ops/running/logging
  - /riak/kv/2.5.0/ops/running/logging
---
...
lager provides a number of configuration options that you can use to fine-tune your Riak cluster's logging output. A compact listing of parameters can be found in our [configuration files](/riak/kv/2.5.0/configuring/reference/#lager) documentation. A more thorough explanation of these options can be found in this document.
...
* `file` --- Console logs will be emitted to a file. This is Riak's
    ...
    `./log/console.log` on an installation from [source](/riak/kv/2.5.0/setup/installing/source), but will differ on platform-specific installation,
...
* `console` --- Console logs will be emitted to standard output, which
    can be viewed by running the [`riak attach-direct`](/riak/kv/2.5.0/using/admin/riak-cli/#attach-direct) command
