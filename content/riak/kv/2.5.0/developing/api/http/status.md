...
toc: true
aliases:
  - /riak/2.5.0/dev/references/http/status
  - /riak/kv/2.5.0/dev/references/http/status
---
...
The output of `/stats` contains the output of `riak-admin status` detailed in the [Inspecting a Node](/riak/kv/2.5.0/using/cluster-operations/inspecting-node) doc, plus the below stats generated by the Riak Core application.
