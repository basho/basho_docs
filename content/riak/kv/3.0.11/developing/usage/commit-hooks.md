...
aliases:
  ...
  - /riak/kv/3.0.11/dev/using/commit-hooks
---
...
Pre- and post-commit hooks are applied at the [bucket]({{<baseurl>}}riak/kv/3.0.11/learn/concepts/buckets) level,
...
Because hooks are defined at the bucket level, you can create [bucket types]({{<baseurl>}}riak/kv/3.0.11/developing/usage/bucket-types)
...
documentation on [object deletion]({{<baseurl>}}riak/kv/3.0.11/using/reference/object-deletion)) to determine whether a delete is
occurring.
...
- `fail` - The atom `fail` will cause Riak to fail the write and send a 403 Forbidden error response (in the [HTTP API]({{<baseurl>}}riak/kv/3.0.11/developing/api/http)) along with a generic error message about why the write was blocked.
