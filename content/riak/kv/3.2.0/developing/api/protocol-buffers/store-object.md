...
aliases:
  ...
  - /riak/kv/3.2.0/dev/references/protocol-buffers/store-object
---

Stores an object under the specified location, as determined by the
intended [key]({{<baseurl>}}riak/kv/3.2.0/learn/concepts/keys-and-objects), [bucket]({{<baseurl>}}riak/kv/3.2.0/learn/concepts/buckets), and [bucket type]({{<baseurl>}}riak/kv/3.2.0/developing/usage/bucket-types). A bucket must always be specified (via
...
[bucket type]({{<baseurl>}}riak/kv/3.2.0/developing/usage/bucket-types) is assigned, Riak will assign
`default`, which means that the [default bucket configuration]({{<baseurl>}}riak/kv/3.2.0/configuring/reference/#default-bucket-properties) will be used.
...
`content` | The new or updated contented of the object. Uses the same `RpbContent` message returned as part of an `RpbGetResp` message, documented in [PBC Fetch Object]({{<baseurl>}}riak/kv/3.2.0/developing/api/protocol-buffers/fetch-object)
...
as well as the object's [causal context]({{<baseurl>}}riak/kv/3.2.0/learn/concepts/causal-context), in the `vclock`
