...
aliases:
  ...
  - /riak/kv/2.9.10/ops/running/handoff/
---
...
It occurs when a [vnode]({{<baseurl>}}riak/kv/2.9.10/learn/glossary/#vnode) no longer belongs to the node on which it's running. This typically happens when the very
...
[configuration files]({{<baseurl>}}riak/kv/2.9.10/configuring/reference/#intra-cluster-handoff)
...
If you're using Riak features such as [Riak Search]({{<baseurl>}}riak/kv/2.9.10/developing/usage/search/),
...
duration that a [vnode]({{<baseurl>}}riak/kv/2.9.10/learn/glossary/#vnode) can be blocked by multiplying the
`handoff.max_rejects` setting by the value of
[`vnode_management_timer`]({{<baseurl>}}riak/kv/2.9.10/configuring/reference/#vnode_management_timer).
