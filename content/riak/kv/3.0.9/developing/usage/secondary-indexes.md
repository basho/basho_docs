...
recommend [Riak Search]({{<baseurl>}}riak/kv/3.0.9/developing/usage/search/) rather than secondary indexes for
...
[Riak Search]({{<baseurl>}}riak/kv/3.0.9/developing/usage/search/) serves analogous purposes but is quite
...
mechanism that does not require a schema (as does [Riak Search]({{<baseurl>}}riak/kv/3.0.9/using/reference/search/#schemas)) and a basic query interface, i.e. an interface that
...
* When you need more than the exact match and range searches that 2i
  supports. If that's the case, we recommend checking out [Riak Search]({{<baseurl>}}riak/kv/3.0.9/developing/usage/search/).
