...
your Riak cluster (which is different from [recovering a failed node]({{<baseurl>}}riak/kv/3.0.9/using/repair-recovery)). Here is the recommended way to go
...
3. Start the new `riak7` node with [`riak start`]({{<baseurl>}}riak/kv/3.0.9/using/admin/riak-cli/#start):
...
participating in the cluster; for example `riak0` with the [`riak admin cluster join`]({{<baseurl>}}riak/kv/3.0.9/using/admin/riak admin/#cluster) command executed on the new `riak7` node:
...
`riak7` node using the [`riak admin cluster replace`]({{<baseurl>}}riak/kv/3.0.9/using/admin/riak admin/#cluster) command:
...
6. Examine the proposed cluster changes with the [`riak admin cluster plan`]({{<baseurl>}}riak/kv/3.0.9/using/admin/riak admin/#cluster) command executed on the new
`riak7` node:
...
[`riak admin cluster commit`]({{<baseurl>}}riak/kv/3.0.9/using/admin/riak admin/#cluster) command:

    ```bash
    riak admin cluster commit
    ```

    If you need to clear the proposed plan and start over, use [`riak admin cluster clear`]({{<baseurl>}}riak/kv/3.0.9/using/admin/riak admin/#cluster):
...
with the [`riak admin ringready`]({{<baseurl>}}riak/kv/3.0.9/using/admin/riak admin/#ringready)
and [`riak admin member-status`]({{<baseurl>}}riak/kv/3.0.9/using/admin/riak admin/#member-status)
commands.
