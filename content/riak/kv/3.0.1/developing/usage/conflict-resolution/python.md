...
aliases:
  ...
  - /riak/kv/3.0.1/dev/using/conflict-resolution/python
...
application will be stored in buckets that bear the [bucket type]({{<baseurl>}}riak/kv/3.0.1/developing/usage/bucket-types) `siblings`, and for this bucket type `allow_mult` is set
...
from the official Python client library, in the [Developing with Riak KV: Usage]({{<baseurl>}}riak/kv/3.0.1/developing/usage) section.
