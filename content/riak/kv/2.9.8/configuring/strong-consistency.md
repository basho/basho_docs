...
`Validation` | This will display `strong` if the `tree_validation` setting in <code><a href="{{<baseurl>}}riak/kv/2.9.8/configuring/reference/#strong-consistency) has been set to `on` and `weak` if set to `off`.
