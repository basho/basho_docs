...
aliases:
  ...
  - /riak/kv/3.0.3/ops/building/basic-cluster-setup
---
...
Most configuration changes will be applied to the [configuration file]({{<baseurl>}}riak/kv/3.0.3/configuring/reference/) located in your `rel/riak/etc` directory (if
...
you'll be using the default port (8087). If you're using the [Protocol Buffers interface]({{<baseurl>}}riak/kv/3.0.3/developing/api/protocol-buffers/) to Riak (which we recommend over the HTTP
...
can also find more detailed instructions about [adding and removing nodes]({{<baseurl>}}riak/kv/3.0.3/using/cluster-operations/adding-removing-nodes) from a cluster.
