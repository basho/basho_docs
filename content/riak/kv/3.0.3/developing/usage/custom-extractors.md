...
toc: true
aliases:
  - /riak/3.0.3/dev/search/custom-extractors
  - /riak/kv/3.0.3/dev/search/custom-extractors
---
...
There are also built-in extractors for [Riak Data Types]({{<baseurl>}}riak/kv/3.0.3/developing/usage/searching-data-types).
...
can be found in our documentation on [advanced]({{<baseurl>}}riak/kv/3.0.3/configuring/reference/#advanced-configuration)). Here's an
example:
...
create a schema that extends the [default schema]({{<baseurl>}}riak/kv/3.0.3/developing/usage/search-schemas/#creating-a-custom-schema). The following fields should be added
...
Now, we can create and activate a [bucket type]({{<baseurl>}}riak/kv/3.0.3/developing/usage/bucket-types)
