...
We'll be using the bucket `Users` to store our data. We won't be [using bucket types](/riak/kv/2.0.7/developing/usage/bucket-types) here, so we don't need to specify one.
