...
[config reference#strong-cons]: /riak/kv/2.0.7/configuring/reference/#strong-consistency
...
[use admin riak-admin#ensemble]: /riak/kv/2.0.7/using/admin/riak-admin/#riak-admin-ensemble-status
...
`Validation` | This will display `strong` if the `tree_validation` setting in <code><a href="http://docs.basho.com/riak/kv/2.0.7/configuring/reference/#strong-consistency">riak.conf</a></code> has been set to `on` and `weak` if set to `off`.
