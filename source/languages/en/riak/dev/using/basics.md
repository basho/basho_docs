...
```bash
$ curl -v http://127.0.0.1:8089/riak/test/doc2
```
...
```
$ curl -v -XPUT http://127.0.0.1:8089/riak/test/doc?returnbody=true \
...
```
$ curl -v -XPOST http://127.0.0.1:8089/riak/test \
  -H 'Content-Type: text/plain' \
  -d 'this is a test'
```

In the output, the *Location* header will give the you key for that object. To view the newly created object, go to `http://127.0.0.1:8089/*_Location_*` in your browser.
...
```bash
$ curl -v -XDELETE http://127.0.0.1:8089/riak/test/test2
```
...
```
$ curl -v -XPUT http://127.0.0.1:8089/riak/test \
  -H "Content-Type: application/json" \
  -d '{"props":{"n_val":5}}'
```
...
```bash
$ curl -v http://127.0.0.1:8089/riak/test
```

You can also view this bucket information through any browser by going to `http://127.0.0.1:8089/riak/test`
