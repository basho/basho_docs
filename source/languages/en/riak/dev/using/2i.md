...
# In the Ruby client, if you do not specify a bucket type, it will use the
# default type. And so this store command would be equivalent to the one
# above:
...
# In the Python client, if you do not specify a bucket type, it will use the
# default type. And so this bucket would bear the default type:
bucket = client.bucket('users')
...
obj.add_index('email_bin', 'jsmith')
obj.store()
```
...
bucket.get_index('twitter_bin', 'jsmith').results
```
