...
(in leveldb log) Compaction error | Turn off the node and run repair on the levelDB partition. See <a href="/references/Errors/#f2">2</a>
...
2. <a name="f2"></a>Run LevelDB compaction
  ...
  2. Start an Erlang session (do not start riak, we just want Erlang)
  3. From the erlang console perform the following command to open the LevelDB database
        ...
        ```
  4. For each of the corrupted LevelDB databases (found by `find . -name "LOG" -exec` | `grep -l 'Compaction error' {} \; `) run this command substituting in the proper vnode number.
    
        ```erlang
        eleveldb:repair("/var/db/riak/leveldb/442446784738847563128068650529343492278651453440", []).
        ```
  5. When all have finished successfully you may restart the node: `riak start`
  6. Check for proper operation by looking at log files in /var/log/riak and in the LOG files in the effected LevelDB vnodes.
