...
be best to start with our general guide to [[authentication and
authorization]].
...
[[created|Authentication and Authorization#User-Management]] and that
security has been [[enabled|Authentication and
Authorization#The-Basics]].
...
interface, and can be used only through the [[protocol buffers
interface|PBC API]].
</div>
...
a different directory in your `[[riak.conf|Configuration Files#]]` by
either uncommenting those lines if you choose to use the defaults or
setting the paths yourself:
...
find the node's name in `[[riak.conf|Configuration Files]]` under the
parameter `nodename`. And so if the `nodename` for a cluster is
...
the nodes in your Riak cluster, you need to perform a [[rolling
restart]]. Once that process is complete, you can use the client
