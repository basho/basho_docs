...
The values for the fragmentation and dead bytes thresholds
_must be equal to or less than their corresponding trigger values_. If
they are set higher, Bitcask will trigger merges in cases where no files
meet the threshold, which means that Bitcask will never resolve the
conditions that triggered merging in the first place.
</div>
