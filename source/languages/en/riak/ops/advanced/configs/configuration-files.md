...
The `app.config` file is used to set various attributes for the Riak node, such as the storage backend that the node will use to store data. The `vm.args` file is used to pass parameters to the Erlang node, such as the name or cookie of the Erlang node.
...
The storage backend that Riak should use is set using the `storage_backend` property. Riak will refuse to start if no storage backend is specified. Here are the available backends:
