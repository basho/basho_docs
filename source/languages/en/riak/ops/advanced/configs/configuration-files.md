...
`pb_port` | The port to which the Protocol Buffers interface will bind. | `8087` | {{1.2.0-}}
`pb_backlog` | The maximum length to which the queue of pending connections may grow. If set, it must be an integer >= 0. If you anticipate a huge number of connections being initialized simultaneously, set this number higher. | `5` | {{1.2.0-}}
...
`webmachine_logger_module` | This needs to be set in order to enable access logs. Example:<br /><br />`{webmachine, [{webmachine_logger_module, webmachine_logger}]}`<br />**Note**: The additional disk I/O of an access log imposes a performance cost you may not wish to pay. Therefore, by default, Riak does not produce access logs. | **none** |

### Riak Backends

The storage backend that Riak should use is set using the `storage_backend` property. Riak will refuse to start if no storage backend is specified. Available backends:
...
This will use async messaging until the mailbox exceeds 20 messages, at which point synchronous messaging will be used, and then switch back to asynchronous when size is reduced to 20 - 5 = 15.
...
`colors` | Configure the colors to use for console messages, using ANSI escape sequences. The default colors are as follows:<br /><br />
```erlang
{colors, [
    {debug,     "\e[0;38m" },
    {info,      "\e[1;37m" },
...
These properties are for buckets that have not been explicitly defined. Imagine a scenario, for example, in which you create two buckets with explicitly defined properties but know that you'll end up using other buckets beyond the initial two. If you'd like to define the properties of *those* additional buckets, you would set properties under the `default_bucket_props`, which is constructed as a list of Erlang tuples (along the lines of `[{prop1,value},{prop2,value}]`. Here's an example:
...
The format of the `vm.args` file is fairly loose. Lines that do not begin with `#` are concatenated and passed to the `erl` on the command line as is.
