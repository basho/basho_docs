...
Access stats are tracked on a per-user basis, as rollups for slices of time. They are stored in the same Riak cluster as other Riak CS data in the `cs.access` bucket.
...
Log retrieval then involves simply making a request to Riak for all slice
...
By default, the script will wait up to 50 seconds for the logger to acknowledge that it has passed its accumulation to the archiver and
...
1.  Storage is calculated for all users either
    1.  on a regular schedule or
    2.  when manually triggered with the `riak-cs-storage` script.
...
If you would like to have an Riak CS node calculate the storage used by every user at the same time (or times) each day, specify a schedule in that node's Riak CS `app.config` file.
...
The period for storage archival is separate from the period for access archival. The storage archival period is configured by the application environment variable `storage_archive_period`. The default is 86400 (one
day). This is because storage calculations are expected to be archived much less frequently than access logs, and so specifying fewer possible keys to look up later reduces overhead at reporting time.
