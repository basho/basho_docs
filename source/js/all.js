...
(typeof jQuery !== 'function') ? null : jQuery(function () {
  ...
  */
  ...
  /*
   * toggleBannerAd()
   * Hide or show the banner add depending on the value
   * for displayAd in local storage
   */

  function toggleBannerAd() {
    var tempBanner = $('#ricon-temp-banner').first()[0];
    tempBanner.hidden = true;

    var displayAd = window.localStorage.getItem('displayAd') || 'false';

    if (displayAd != 'false') {
      tempBanner.hidden = false;
    }

    tempBanner.click(function() {
      $(this).hide();
      window.localStorage.setItem('displayAd', 'false');
    });
  }
